...
export class HomeComponent implements OnInit {
  ...
  public groupBySerialNumber(locations: Location[]): { [key: string]: Location[] } {
    return locations.reduce((groups: { [key: string]: Location[] }, location) => {
  ...
  private addLocationToMap(name: string, locations: Location[]): void {
    ...
    locations.forEach(location => {
      const latLng: L.LatLngExpression = [location.locationlatitude!, location.locationlongitude!];
  ...
  public onLocationGroupClick(serialNumber: string): void {
    console.log(`Clicked on location group with serial number: ${serialNumber}`);
